...
### Create Group
##### 【command format】
28672 groups can be created in the mesh network. The id range from 8000 to EFFF (hexadecimal) is maintained locally.

#####  【Method Invocation】
```java
* @param name			group name
* @param pcc			category of devices in the group 
* @param localId	localId of the group (range 8000 ~ EFFF hex string)  
* @param callback		
...
##### 【Example Codes】
```java
mIWiserBlueMesh.addGroup("group name","category", "8001", new IAddGroupCallback() {
			@Override
            public void onError(String errorCode, String errorMsg) {
